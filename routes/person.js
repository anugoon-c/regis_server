...
router.get("/", function (req, res, next) {
  ...
  let sql = `SELECT * FROM person_dev WHERE YEARIN = '4/61' AND BATT = '3' AND COMPANY = '4'`;
  let query = db.query(sql, (err, results) => {
    ...
    results.length === 0
      ? res.status(400).send("ไม่พบข้อมูลทหารในระบบ")
      : res.json(results);
  });
});
...
router.post("/create", async function (req, res, next) {
  let sql = `   INSERT INTO person_dev 
            (   BATT, 
                COMPANY, 
                PLATOON,
                PSEQ, 
  ...
  try {
    ...
    let sql = ` UPDATE person_dev
                SET 
                  ...
                  ADDRESS_MU = '${req.body.ADDRESS_MU}',
                  ADDRESS_SOIL = ${req.body.ADDRESS_SOIL},
                  ...
                  OCCCODE = '${req.body.OCCCODE}',
                  SKILLCODE = '${req.body.SKILLCODE}',
                  SKILL = '${req.body.SKILL}',
                  EDUCODE1 = '${req.body.EDUCODE1}',
                  EDUCODE2 = '${req.body.EDUCODE2}'
                WHERE NAVYID = '${req.params.id}'`;
    let query = db.query(sql, (err, results) => {
      if (err) {
        res.status(400).send("ไม่สามารถแก้ไขข้อมูลทหารได้");
        throw err;
      }
      res.send("แก้ไขข้อมูลทหารสำเร็จ");
    });
  } catch (err) {
...
router.delete("/:id", function (req, res, next) {
  ...
  let query = db.query(sql, (err, results) => {
    ...
    results.affectedRows === 0
      ? res.status(400).send("ไม่พบข้อมูลทหารในระบบ")
      : res.send(`ลบข้อมูล NAVYID: ${req.params.id} เรียบร้อย`);
  });
});
